    ...
    class Program
    {
        ...
        static void unigram(string  hh)
        {
            ...
            for (int i = 0; i < testwordcount; i++)
            {
                ...
                {
                    temp2 += Math.Log10((double)(1) / (double)(num + res.Count));//(1) / (N+V)
                }
                else
                {
                    temp2 += Math.Log10((double)(res[testword[i]] + 1) / (double)(num + res.Count));//(c(wx)+1) / (N+V)
                }
                prolog += temp2;
            ...
            for (int i = 0; i < testwordcount; i++)
            {
                ...
                if (cc1 == 1)
                {
                    ...
                    if (!res2.TryGetValue(biword1[0] + "~" + biword1[1], out actualValue))
                    {
                        ...
                        {
                            temp2 += Math.Log10((double)(1) / (double)(res[biword1[0]] + res.Count));//(1) / (c(wn-1)+V) 不算太糟的情形
                        }
                        else 
                        {
                            temp2 += Math.Log10((double)(1) / (double)(res.Count));//(1) / (V) 最糟的情形
                        }
                    }
                    else
                    {
                        temp2 += Math.Log10((double)(res2[biword1[0] + "~" + biword1[1]] + 1) / (double)(res[biword1[0]] + res.Count));//(1+c(wn-1,wn)) / (c(wn-1)+V) 最好的情形
                    }
                    prolog += temp2;
