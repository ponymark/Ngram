...
namespace ConsoleApplication1
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //2.正則表達式必須繼續改進 考慮更多狀況 或是簡化整合式子 例如把"="拿掉不抓
        ...
        static void bigram(string hh)
        {
            ...
            for (int i = 0; i < testwordcount; i++)
            {
                ...
                if (cc1 == 1)
                {
                    ...
                    double temp2 = 0;
                    int actualValue = 0;
                    if (!res.TryGetValue(biword1[1], out actualValue))
                    {
                        temp2 += Math.Log10((double)(1) / (double)(res[biword1[0]] + res.Count));
                    }
                    else
                    {
                        temp2 += Math.Log10((double)(res2[biword1[0] + "~" + biword1[1]] + 1) / (double)(res[biword1[0]] + res.Count));
                    }
                    prolog += temp2;
